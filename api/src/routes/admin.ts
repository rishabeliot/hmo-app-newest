...
// POST /admin/tickets/force-confirm-by-email
// Convenience: takes { email, event_id } — finds/creates the ticket and runs full confirm
router.post('/tickets/force-confirm-by-email', requireAuth, async (req: Request, res: Response): Promise<void> => {
  if (!guardAdmin(req, res)) return;

  const { email, event_id } = req.body;
  if (!email || !event_id) {
    res.status(400).json({ error: 'email and event_id are required' });
    return;
  }
  ...
  const existingTickets = await db
    .select()
    .from(tickets)
    .where(and(eq(tickets.eventId, event_id), eq(tickets.userId, user.id)));

  let ticketId: string;
  if (existingTickets.length > 0) {
    ticketId = existingTickets[0].id;
    if (existingTickets[0].bookingStatus === 'confirmed') {
      res.status(200).json({ ok: true, message: 'Already confirmed', ticket_id: ticketId });
      return;
    }
  } else {
    const [inserted] = await db
      .insert(tickets)
      .values({ eventId: event_id, userId: user.id, bookingStatus: 'pending' })
      .returning({ id: tickets.id });
    ticketId = inserted.id;
  }

  const qrCodeToken = jwt.sign(
    { ticket_id: ticketId, event_id, user_id: user.id, issued_at: Date.now() },
    process.env.QR_HMAC_SECRET!,
    { expiresIn: '30d' }
  );
  ...
  const [[event], qrBuffer] = await Promise.all([
  ...
  res.status(200).json({ ok: true, ticket_id: ticketId });
});
